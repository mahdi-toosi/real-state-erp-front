...
export default {
	updated(el, binding) {
		...
		const block = binding.value && binding.value.block ? binding.value.block : null
		const delimiter = binding.value && binding.value.delimiter ? binding.value.delimiter : null
		...
		if (element.value) {
			...
			if (elementValue.length === 1) {
				...
				const previousValue = elementValue.substring(0, elementValue.length - 1)
				const latestCharacter = elementValue.substring(elementValue.length - 1, elementValue.length)
				const convertedValue = toEnNumber(
					testCharacter(latestCharacter) ? elementValue : previousValue
				)
				element.value =
					block && delimiter ? addDelimiter(convertedValue, block, delimiter) : convertedValue
			}
		}
...
export function testCharacter(char) {
...
function addDelimiter(str) {
	const num = str.toString().split('.')
	return num.length > 1
		? Number(num[0]).toLocaleString() + '.' + num[1]
...
export function toEnNumber(number) {
	...
	const enStr = Array.from(str).reduce((prev, curr) => `${prev}${toEn[curr] || curr}`, '')
	return enStr
}
